...
export function ActiveRequestCard({
  ...
  return (
    <section className="border-warning overflow-hidden rounded-2xl border-[1.5px] shadow-[0px_2px_8px_0px_rgba(18,23,38,0.04)]">
      {/* 헤더 */}
      <div className="bg-warning-bg border-warning flex items-center justify-between border-b px-6 py-[18px]">
        <div className="flex items-center gap-2.5">
          <span className="bg-surface text-warning flex size-9 items-center justify-center rounded-[10px] text-[15px] font-bold">
            ⚠
          </span>
          <div className="flex flex-col gap-0.5">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-warning rounded-[5px] px-[7px] py-[3px] text-[11px] font-bold text-white">
                조정 제안 — 응답 대기
              </span>
            </div>
            <span className="text-fg-muted text-[12px]">
              멘토가 다른 일정·장소를 제안했습니다. 수락하거나 요청을 취소할 수
              있습니다.
            </span>
          </div>
        </div>
      ...
      <div className="bg-surface flex items-stretch gap-3 px-6 py-[18px]">
        <SlotCard
          ...
          memoLabel="요청 메모"
        />
        <div className="text-fg-subtle flex items-center justify-center">→</div>
        {request.proposal && (
          <SlotCard
      ...
      <div className="bg-surface border-divider flex items-center justify-between border-t px-6 pt-3.5 pb-[18px]">
        <span className="text-fg-subtle text-[11px] font-medium">
          제안 받은 시각 {request.proposedAtLabel} · 24시간 내 응답 권장
        </span>
        <div className="flex items-center gap-2">
          <button
            type="button"
