...
export default function HrdApiKeyPage() {
  ...
  if (listQuery.isError || !listQuery.data) {
    // HRD는 실 BE(learning-service) 전용. mock 모드(기본 dev)에선 mock 토큰이라 401이 난다.
    const status = isAxiosError(listQuery.error)
      ? listQuery.error.response?.status
      : undefined
    const realAuth = import.meta.env.VITE_REAL_AUTH === 'true'
    const { title, description } = !realAuth
      ? {
          title: 'HRD API Key는 실 BE 전용이에요',
          description:
  ...
  const activeCount = summary?.activeKeys ?? keys.filter((k) => k.active).length
    ...
    <div className="p-8">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1fr_380px]">
        <div>
          <div className="mb-2 flex items-end justify-between">
            ...
            <StatusBadge label={`총 ${list.totalElements}건`} tone="neutral" />
          </div>
          <DataTable
            columns={keyColumns}
            rows={keys}
      ...
      <div className="mt-6">
        ...
        <DataTable
          ...
          empty={
            historyQuery.isError ? '이력을 불러오지 못했어요' : '이력이 없어요'
          }
        />
