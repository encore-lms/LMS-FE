...
export default function EndorsementDetailPage() {
  ...
  if (isPending) {
    return <div className="text-fg-muted p-8">추천서를 불러오는 중…</div>
  }
  if (isError || !data) {
    return (
      <div className="p-8">
  ...
  const meta = SNAPSHOT_META[data.snapshotStatus]
    ...
    <div className="p-8">
      ...
      <div className="border-info/30 bg-info-bg flex gap-3 rounded-xl border p-4">
        ...
        <div>
          ...
          <p className="text-fg-muted mt-0.5 text-xs">
            24시간 이후 변경은 신규 row로 저장됩니다. 외부 공개는 증명서 최신화
            작업 이후 공개 스냅샷에 반영됩니다.
          </p>
        </div>
      </div>
      ...
      <div className="border-border bg-surface mt-4 flex items-center gap-4 rounded-xl border p-5">
        ...
        <div className="flex flex-col">
          <span className="text-fg text-sm font-bold">{data.student.name}</span>
          ...
          {data.student.email && (
            <span className="text-fg-subtle text-xs">{data.student.email}</span>
          )}
        </div>
      ...
      >
        ...
        <div className="mt-5">
          ...
          <div className="border-border bg-surface-muted mt-2 rounded-lg border p-4">
            ...
            <p className="text-fg-muted mt-1 text-xs">
              수정 내용은 다음 증명서 최신화 작업 이후 공개 스냅샷에 반영됩니다.
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          <span className="text-fg-subtle text-xs">
            수정 사항은 다음 증명서 최신화 작업 후 외부 공개에 반영됩니다 · 24h
            이후 변경은 신규 row
          </span>
          <div className="flex gap-2">
            <Button
