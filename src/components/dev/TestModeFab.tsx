...
// 평소엔 정식 화면을 가리지 않게 버튼만 떠 있고, 짧게 누르면 시뮬레이션 패널이 펼쳐진다(드래그하면 이동).
...
export function TestModeFab({
  ...
  // 버튼이 화면 어느 쪽에 있느냐에 따라 패널이 펼쳐질 방향을 정한다(밖으로 안 넘치게).
  const openUp = pos.y + FAB_SIZE / 2 > window.innerHeight / 2
  const openLeft = pos.x + FAB_SIZE / 2 > window.innerWidth / 2

  return createPortal(
    <div className="fixed z-40 h-14 w-14" style={{ left: pos.x, top: pos.y }}>
      {open && (
        <div
          className={cn(
