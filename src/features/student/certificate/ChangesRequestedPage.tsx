...
export default function ChangesRequestedPage() {
  ...
  const doneCount = (data?.checklist ?? []).filter((c) =>
    isDone(c.id, c.done),
  ).length
  const allDone = doneCount === (data?.checklist.length ?? 0)
  const doneLabel = `${doneCount} / ${data?.checklist.length ?? 0}`
                  ...
                  <div className="flex items-center gap-3 px-5 py-3.5">
                    ...
                    {c.actionLabel && (
                      <button
                        type="button"
                        onClick={() =>
                          navigate(areaRoute(c.actionLabel as string))
                        }
          ...
          <div className="bg-brand-deep fixed right-8 bottom-6 left-[232px] z-30 flex items-center justify-between rounded-2xl px-6 py-4 text-white shadow-[0px_12px_32px_0px_rgba(18,23,38,0.28)]">
            <div className="flex flex-col">
              <span className="text-[13px] font-bold">
                재요청 전 체크 {doneLabel} 완료 — 모든 보완 사유 처리 후 재요청
                가능
              </span>
